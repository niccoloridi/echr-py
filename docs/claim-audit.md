...
Checked on **21 August 2026**. This audit separates facts verified from this
...
| --- | --- |
| Python 3.11–3.14, package 0.2.1 | `pyproject.toml`, runtime version, CI matrix, wheel build, and isolated installation |
...
| Inclusive and paragraph citation artifacts | v3 locus invariance, compound procedural groups, structured footnote invocations, occurrence/opinion/pinpoint ownership, target mapping, CLI/MCP parity, and offline tests |
...
The current release-preparation pass contains 803 passing offline tests and six
...
| Mumford annotations | Pinned revision `166b15c31276bfa8c8775e7e7def575e916aced7`; [repository](https://github.com/jamumford/ECHR-citation-context-v1) declares CC0-1.0. Local import found 115 curated XMI documents, 5,481 curated annotations, and 8,899 individual-annotator annotations. After excluding 56 annotations outside the ECHR reference class, the final v3 run at `c084bbfc306acb2462cbc2adc0b29c985142274e` aligned 5,319/5,425 selected ECHR annotations one-to-one (98.0%): 5,316 by strict source-span overlap and three by uniquely normalized context, with 86 ambiguity abstentions and 20 unmatched annotations. This is selected-annotation recovery, not detector precision or judgment-level recall. Exact-document resolution covered 4,629/5,319 aligned references; the reference does not supply canonical target item IDs, so this is coverage rather than independently verified accuracy. Printed application-number identification covered 2,412/2,615 aligned references carrying a number, and none of those identifications conflicted with the printed number; this does not establish exact procedural-document accuracy. Complete target-paragraph mapping covered 3,794/4,543 aligned references carrying local pinpoints; the reference supplies no target-paragraph links, so this is also coverage. The [methodology audit](mumford-methodology-audit.md) explains the scope and abstentions. Exact input, code, artifact, and denominator hashes are recorded in the [full-inclusive audit](benchmarks/mumford-full-inclusive-audit.json); two clean detached-worktree runs reproduced the frozen core artifacts byte-for-byte. |
...
2. rerun the full test/wheel matrix;
