...
The current release-preparation pass contains 673 passing offline tests and six
...
| Mumford annotations | Pinned revision `166b15c31276bfa8c8775e7e7def575e916aced7`; [repository](https://github.com/jamumford/ECHR-citation-context-v1) declares CC0-1.0. Local import found 115 curated XMI documents, 5,481 curated annotations, and 8,899 individual-annotator annotations. The historical 78.2% figure is an occurrence-reusing identity/context proxy, not strict span recovery. The final identity-gated text-only run aligns 2,576/5,481 annotations (47.0%), with 1,934 ambiguity abstentions and 971 unmatched annotations. At evaluated revision `930cbbfbae57f4d742917f2ecfab17cd6896bc05`, the final v3 full-inclusive run aligns 4,662/5,425 selected ECHR labels (85.9%) after unique XMI-Sofa projection, with 385 ambiguity abstentions and 378 unmatched annotations. This is selected-annotation recovery, not detector precision. Exact-document identification for 4,462/4,662 aligned references is automatic coverage, not accuracy, because the reference does not supply canonical target item IDs. Printed application numbers were identified for 2,363/2,458 references that supplied one, with 2,363/2,363 conditional agreement and no conflicts; this does not establish exact procedural-document accuracy. Complete target-paragraph mapping for 2,588/3,983 local pinpoints is likewise coverage because the reference supplies no target-paragraph links. The [methodology audit](mumford-methodology-audit.md) explains the scope, abstentions, and a source-document grouping defect in the published repetition analysis. Exact input, code, artifact, and denominator hashes are recorded in the [full-inclusive audit](benchmarks/mumford-full-inclusive-audit.json); two clean detached-worktree runs reproduced all six frozen output artifacts byte-for-byte. |
